...
import { currentSetup, getSetupAsEmbed, hosted, video } from "../setup";
import {
  getHistory,
...
export async function startGame(
  ...
  const allowedPlayerCount = maxplayers
  ...
  state.playerrole = channel.guild.roles.find(
    (role) => role.name === config.player_role
  );
  ...
  if (setupName === "moderated") {
    ...
    Core.waitWithCheck(() => !state.isGameInSignups(), 10, 7200).then(
      async (isFulfilled) => {
        if (!isFulfilled) {
          channel.send(
            `Sorry ${user.displayName}, your moderated game timed out after two hours.`
          );
    ...
    channel.send(
      `Starting a${setup.testGame ? "n unrated" : ""} game of ${
        setup.video ? "Video " : ""
      }Mafia [${setupName} for ${allowedPlayerCount} players] in ${
        timer / 60
      } minutes. Type \`!in\` to sign up.`
    );
    Core.waitWithCheck(
      () => state.isGameInProgress() || state.isGameOver(),
      5,
      300
    ).then(async (isFulfilled) => {
      if (isFulfilled) {
        return;
      }
      if (
...
export async function playerIn(
  ...
  if (
    state.players.find((player) =>
      player.displayName.includes(user.displayName)
    )
  ) {
    // issue with my lazy form of display name parsing...
    const duplicateName = state.players.find((player) =>
      player.displayName.includes(user.displayName)
    ).displayName;
    channel.send(
      `I'm sorry, but your name is too similar to ${duplicateName}'s.\nIf this is shitty for you, please complain to Urist so he knows to fix it! Otherwise he won't bother!`
    );
    return;
  }
  const BANNED_NAMES = [
  ...
  if (BANNED_NAMES.includes(user.displayName.toLowerCase())) {
    channel.send(
      `That's sooooooo funny. You're gonna mess stuff up if you join a game with that nickname, ${user.displayName}.`
    );
    return;
  }
  if (
    currentSetup.maxplayers &&
    state.players.length >= currentSetup.maxplayers
  ) {
  ...
  if (currentSetup.name === "test" || hosted) {
    let role: MafiaRole;
    let team: MafiaTeam;
    if (args[0]) {
      role = getRole(args[0]);
    ...
    state.players.find((p) => p.id === user.id).mafia = new MafiaPlayer(
      role,
      team
    );
  }
...
export async function playerOut(
  channel: TextChannel,
  user: GuildMember
): Promise<void> {
  if (!state.isGameInSignups()) {
  ...
  if (
    !state.players.find((player) => player.displayName === user.displayName)
  ) {
  ...
  channel.send(
    `You are no longer signed up for the next game, ${user.displayName}.`
  );
}

export async function players(channel: TextChannel): Promise<void> {
  channel.send(
    `Players (${state.players.length}): ${state.players
      .map((player) => Core.findUserMention(channel, player.displayName))
      .join(", ")}`
  );
}
...
export async function spoilers(
  channel: TextChannel,
  user: GuildMember
): Promise<void> {
  ...
  if (accessToken) {
    mafbot.guilds
      .filter((guild) => guild.name.startsWith("MafBot ~ "))
      .forEach((guild) =>
        addUserToGuild(
          user.user,
...
export async function history(user: User, args: string[]): Promise<void> {
  ...
  if (args.length === 0) {
    ...
    if (/^\d+$/.test(arg1)) {
      const embed = await getFullHistoryEmbed(
        await getHistory(1, Number(arg1)),
        false
      );
      user.send(embed);
      return;
    ...
    if (arg1 === "summary") {
      const embed = await getSummaryHistoryEmbed(
        await getHistory(25),
        "Summary: last 25 games"
      );
      user.send(embed);
      return;
    }
    ...
    if (args[0] === "winners") {
      ...
      if (winningTeam === "t" || winningTeam === "town") {
        ...
        user.send(
          `It looks like '${winningTeam}' isn't the name of a team. Is it spelled correctly?`
        );
        return;
      }
    ...
    let userHistory = gameRecord.find(
      (record) => record.username.toLowerCase() === args[0]
    );
    if (!userHistory) {
      userHistory = gameRecord.find((record) =>
        record.username.toLowerCase().startsWith(args[0])
      );
    }
    if (userHistory) {
      ...
      userRecord = guild.members.find(
        (member) => member.displayName.toLowerCase() === args[0]
      );
      if (!userRecord) {
        userRecord = guild.members.find((member) =>
          member.displayName.toLowerCase().startsWith(args[0])
        );
      }
      if (userRecord) {
        ...
        user.send(
          `I couldn't find any user by the name of '${args[0]}' in the ${guild.name} server.`
        );
        return;
      }
    }
    if (gameRecord.find((record) => record.userid === userId)) {
      ...
      if (/^(lynched|l|killed|k)(day|d|night|n)\d+$/.test(unparsedDeath)) {
        ...
        if (unparsedDeath.startsWith("l")) {
          death = "lynched";
          unparsedDeath = unparsedDeath.startsWith("lynched")
            ? unparsedDeath.substring(7)
            : unparsedDeath.substring(1);
        } else if (unparsedDeath.startsWith("k")) {
          death = "killed";
          unparsedDeath = unparsedDeath.startsWith("killed")
            ? unparsedDeath.substring(6)
            : unparsedDeath.substring(1);
        }
        if (unparsedDeath.startsWith("d")) {
          phase = "day";
          unparsedDeath = unparsedDeath.startsWith("day")
            ? unparsedDeath.substring(3)
            : unparsedDeath.substring(1);
        } else if (unparsedDeath.startsWith("n")) {
          phase = "night";
          unparsedDeath = unparsedDeath.startsWith("night")
            ? unparsedDeath.substring(5)
            : unparsedDeath.substring(1);
        }
        cycle = unparsedDeath;
        if (death && phase && cycle) {
          ...
          await user.send(
            `Change confirmed, ${
              userHistory ? userHistory.username : userRecord.displayName
            } was ${parsedDeath}.`
          );
          await history(user, [gameId ? String(gameId) : "last"]);
          return;
        ...
        user.send(
          `You screwed up your death method string. It should be something like \`lynchedday1\` or \`kn2\`, but you sent '${args[1]}'.`
        );
        return;
      }
    } else {
      user.send(
        `It looks like ${
          userHistory ? userHistory.username : userRecord.displayName
        } didn't play in that game. If that's wrong then I give you permission to yell at Urist.`
      );
      return;
    }
  }
...
async function getSummaryHistoryEmbed(
  history: History[],
  title: string
): Promise<RichEmbed> {
  let gameNum = 0;
  ...
  function addField(embed: RichEmbed, game: History[]): void {
    ...
    const winningTeam = game[0].winningteam
      ? ` - ${game[0].winningteam} win`
      : ``;
    const setupTitle = `${gameNumber} (${timestamp})\n${setupName}${winningTeam}`;
    const players = game
      .sort((a, b) =>
        a.team > b.team ? 1 : a.team < b.team ? -1 : a.role > b.role ? 1 : -1
      )
      .map(
        (player) =>
          `${
            Core.findUserDisplayNameById(player.guildid, player.userid) ||
            player.username
...
async function getFullHistoryEmbed(
  history: History[],
  last: boolean
): Promise<RichEmbed> {
  history.sort((a, b) =>
  ...
  const winningTeam = history[0].winningteam
    ? ` - ${history[0].winningteam} win`
    : ``;
  const title = last
  ...
  for (const player of history) {
    ...
    teamMembers.push(
      `${
        ...
        player.won !== null
          ? ` - ${
              player.death ? player.death : player.won ? "survived" : "endgamed"
            }`
          : ""
      }`
    );
...
export async function beginGame(channel: TextChannel): Promise<void> {
  if (state.isGameInSignups()) {
    if (
      currentSetup.minplayers &&
      state.players.length < currentSetup.minplayers
    ) {
      channel.send(
        `You should probably wait for at least ${currentSetup.minplayers} to be signed up before trying to start the game.`
      );
      return;
    }
    ...
    if (
      userTokens.length <
      state.players.filter((p) => !isTestUser(p.user)).length
    ) {
      const unauthedPlayers = state.players.filter((player) =>
        userTokens.every((token) => token.userid !== player.id)
      );
      channel.send(
        `I REALLY WANTED TO START THE GAME................................. BUT ${unauthedPlayers
          .map((p) => Core.findUserMention(channel, p.displayName))
          .join(" AND ")} ${
          unauthedPlayers.length > 1 ? `HAVEN'T` : `HASN'T`
        } AUTHENTICATED YET. PERHAPS THEY NEED SOME ASSISTANCE????`
      );
      return;
    }
    await cleanupGuilds();
...
export async function vote(
  ...
  state.votes = state.votes.filter(
    (vote) => vote.voter.displayName !== user.displayName
  );
  let votee: string;
  ...
  if (!vote) {
    ...
    vote.toLowerCase() === "nolynch" ||
    vote.toLowerCase() === "no lynch"
  ) {
    votee = "No Lynch";
  } else {
    const votedPlayer = state.players.find((player) =>
      player.displayName.toLowerCase().startsWith(vote.toLowerCase())
    );
...
export async function forceAction(
  ...
  const player = state.players.find((p) =>
    p.displayName.toLowerCase().startsWith(forcedPlayer)
  );
  const cmd = args.shift();
  await doAction(player.user, args, cmd);
}

export async function forceVote(
  channel: TextChannel,
  user: GuildMember,
  args: string[]
): Promise<void> {
...
export async function addRole(user: User, args: string[]): Promise<void> {
  ...
  if (
    !(team && ["town", "mafia", "sk"].includes(team)) ||
    !roleName ||
    !roleText ||
    args.length
  ) {
    user.send(
      'I think you might have made a mistake. Please use the format `addrole [town|mafia|sk] "[rolename]" "[roletext]"` for this command.'
    );
    return;
  }
...
export async function removeRole(user: User, args: string[]): Promise<void> {
  ...
  const roleToRemove = currentRoles.find(
    (role) => role.role.name.toLowerCase() === removeRoleName.toLowerCase()
  );
  if (!roleToRemove) {
...
export async function modkill(user: User, args: string[]): Promise<void> {
  ...
  args.forEach((arg) => {
    const player = state.players.find((player) =>
      player.displayName.toLowerCase().includes(arg.toLowerCase())
    );
    if (!player) {

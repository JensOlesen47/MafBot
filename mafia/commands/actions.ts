...
import { webSendRoleUpdate } from "../../web/http";
...
export async function doAction(
  ...
  if (!state.isGameInProgress()) {
    user.send(
      `This is not a valid thing to be doing at this time sir and/or madam.`
    );
    ...
    !actioner.mafia.role.abilities.find((ability) => ability.name === cmd) &&
    cmd !== "none"
  ) {
    user.send(
      `Your role can only perform the following action${
        actioner.mafia.role.abilities.length !== 1 ? "s" : ""
      }: ${actioner.mafia.role.abilities
    ...
    cmd === "mafiakill" &&
    actionQueue.find((actn) => actn.name === "mafiakill")
  ) {
    ...
    user.send(
      `Action confirmed: ${cmd}${
        action.victims.length > 0
          ? " on " +
            action.victims.map((victim) => victim.displayName).join(" & ")
          : null
      }.`
    );
  }
...
function getAction(actioner: Player, args: string[], cmd: string): Action {
  ...
  if (args.length !== action.targets.length) {
    actioner.send(
      `You gave ${args.length} target${
        args.length !== 1 ? "s" : null
      } but this action requires ${action.targets.length}.`
    );
    return;
  }
  ...
  args.every((arg, i) => {
    ...
    if (arg === "self") {
      ...
      const victim =
        state.players.find(
          (user) => user.displayName.toLowerCase() === arg.toLowerCase()
        ) ||
        state.players.find((user) =>
          user.displayName.toLowerCase().startsWith(arg.toLowerCase())
        );
      if (victim) {
        if (allowableTargets.includes("unique") && victims.includes(victim)) {
          actioner.send(
            `You cannot target the same player twice with this ability.`
          );
        } else if (allowableTargets.includes("alive") && !victim.mafia.alive) {
          actioner.send(
            `You may only target living players with this ability. ${arg} is dead.`
          );
        } else if (allowableTargets.includes("dead") && victim.mafia.alive) {
          actioner.send(
            `You may only target dead players with this ability. ${arg} is alive.`
          );
        } else {
          victims.push(victim);
          return true;
...
export async function checkForFullActionQueue(): Promise<void> {
  ...
  function playerHasSubmittedAction(player: Player) {
    return (
      ...
      (player.mafia.role.abilities.every(
        (ability) => ability.name === "mafiakill"
      ) &&
        actionQueue.some((action) => action.name === "mafiakill"))
    );
  }
}

export async function deduceActionFromRole(
  status: string,
  player: Player
  ...
  if (fullAction) {
      ...
      switch (target) {
        ...
        default:
          ...
          const targetedPlayers = state.players.filter(
            (i) =>
              (i.mafia.role.truename || i.mafia.role.name) ===
                targetedRole.name && i.id !== player.id
          );
    ...
    console.log(
      `Getting action ${trueActionName} from role with templates ${splitAction}`
    );
    return async (): Promise<void> =>
      await actionFnList.get(trueActionName)(mafiaAbility);
  }
  return async (): Promise<void> => {};
}
...
export async function inspect(action: Action): Promise<void> {
  action.victims.forEach((victim) => {
    const inspectStatus =
      victim.mafia.role.status.inspect || victim.mafia.team.name;
...
export async function transform(action: Action): Promise<void> {
  ...
  action.victims.forEach((victim) => {
    ...
    if (!action.templates.includes("silent")) {
      webSendRoleUpdate(victim);
      victim.send(
        `You have a new role. You are now a ${victim.mafia.role.name} (${victim.mafia.team.name}).`
      );
    }
  });
}
...
export async function reveal(action: Action): Promise<void> {
  state.channel.send(
    `${action.actioner.displayName} has revealed himself to be ${action.actioner.mafia.team.name}!`
  );
}
...
export async function decreasestatus(action: Action): Promise<void> {
  action.victims.forEach((victim) => {
    if (
      victim.mafia.role.status[action.status] &&
      typeof victim.mafia.role.status === "number"
    ) {
...
async function tempDecreaseStatus(
  player: Player,
  status: string
): Promise<void> {
  const action = getAbility("decreasestatus") as Action;
...
async function tempIncreaseStatus(
  player: Player,
  status: string
): Promise<void> {
  const action = getAbility("increasestatus") as Action;

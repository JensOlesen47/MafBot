...
export function getSetupAsEmbed(): RichEmbed {
  ...
  const embed = new RichEmbed()
    .setTitle(`${size}p ${currentSetup.name}`)
    .setDescription(
      'Use `addrole [town|mafia|sk] "[rolename]" "[roletext]"` and `removerole "[rolename]"` to configure your setup.' +
        "\nIf there are more players than roles, the rest will be townies."
    );
  if (currentSetup.fixed) {
    const fixedSetupRoleList = currentSetup.fixedSetups.setups[0].setup.sort(
      (a, b) => (a.team.name > b.team.name ? 1 : -1)
    );
    let lastTeam = "";
      ...
      if (!lastTeam) {
        ...
        embed.addField(
          ...
          teamMembers
            .map(
              (member) =>
                `**${member.role.truename || member.role.name}** - ${
...
export async function initializeSetup(): Promise<void> {
  ...
  logger.debug(
    state.players.map(
      (player) =>
        `${player.displayName} - ${player.mafia.role.name} (${player.mafia.team.name})`
    )
  );
}
...
async function initializeRandomSetup(): Promise<void> {
  ...
  const numScum =
    Math.random() >= 0.5 ? maxScum[numPlayers - 3] : minScum[numPlayers - 3];
...
function selectRoles(
  numPlayers: number,
  numScum: number,
  possibleTownRoles: MafiaRole[],
  possibleMafiaRoles: MafiaRole[]
): MafiaPlayer[] {
  const roleList: MafiaPlayer[] = [];
  const targetTownPower = calculateTownPower(numPlayers, numScum);
  const targetMafiaPower = calculateMafiaPower(
    numPlayers,
    numScum,
    targetTownPower
  );
...
function calculateMafiaPower(
  numPlayers: number,
  numScum: number,
  townPower: number
): number {
  return 0.05 * (numPlayers - numScum) + 0.2 * townPower;
}
...
async function initSetup(): Promise<void> {
  ...
  for (const player of state.players) {
    if (player.mafia.team.abilities) {
      player.mafia.role.abilities = player.mafia.role.abilities.concat(
        player.mafia.team.abilities
      );
    }
    if (player.mafia.role.status.onstart) {
      actionQueue.push(
        await actions.deduceActionFromRole(
          player.mafia.role.status.onstart,
          player
        )
      );
    }
  ...
  for (const player of state.players) {
    const actionString =
      player.mafia.role.abilities.length > 0
        ? player.mafia.role.abilities
            .map(
              (ability) =>
                `${ability.day ? "day" : ""}${ability.name}${
                  ability.shots === 1 ? " (1-shot)" : ""
                }`
            )
            .join(", ")
        : `none`;
    if (player.mafia.role.roletext.includes("BUDDY1")) {
      player.mafia.role.roletext = player.mafia.role.roletext.replace(
        "BUDDY1",
        player.mafia.role.buddy.displayName
      );
    }
    player.send(
      `You are a ${player.mafia.role.name} (${player.mafia.team.name}). ${player.mafia.role.roletext} ${player.mafia.team.wintext}`
    );
    ...
    if (player.mafia.team.openteam) {
      teammates = state.players.filter(
        (p) =>
          p.mafia.team.name === player.mafia.team.name &&
          p.user.id !== player.user.id
      );
    } else if (player.mafia.role.name === "Mason") {
      teammates = state.players.filter(
        (p) => p.mafia.role.name === "Mason" && p.user.id !== player.user.id
      );
    }
    if (teammates.length > 0) {
      player.send(
        `Your ${player.mafia.team.name} budd${
          teammates.length > 1 ? "ies" : "y"
        }: ${teammates.map((mate) => mate.displayName).join(", ")}`
      );
    }
  }
  ...
  const masons = state.players.filter(
    (player) => player.mafia.role.name === "Mason"
  );
  if (masons.length >= 2) {
...
function getFixedSetupRoleList(): MafiaPlayer[] {
  const fixedSetup = currentSetup.fixedSetups.getForPlayers(
